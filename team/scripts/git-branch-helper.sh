...
case "${1:-}" in
    ...
    prepare)
        ...
        if [[ "$CHECK_UNCOMMITTED" == "true" ]]; then
            if ! git diff-index --quiet HEAD -- 2>/dev/null; then
                if [[ "$STASH_BEFORE" == "true" ]]; then
                    ...
                    echo "💾 Stash saved. You can restore later with 'git stash pop'."
                else
        ...
        if git show-ref --verify --quiet "refs/heads/$BRANCH_NAME"; then
            ...
            if [[ "$AUTO_CREATE" == "true" ]]; then
                ...
                echo "   Set auto_create to true in .config/git-workflow.json or"
                echo "   manually create the branch: git checkout -b $BRANCH_NAME $BASE_BRANCH"
                exit 1
            fi
        fi
    ...
    status)
        ...
        echo "📌 Base branch (config): $BASE_BRANCH"
        echo ""
    ...
    # ── cleanup: Cleanup after work ──────────────────────────
    cleanup)
        echo "🧹 Branch cleanup should be done manually."
        echo "   - Delete unnecessary branch: git branch -d <branch-name>"
        echo "   - Delete remote branch: git push origin --delete <branch-name>"
        ;;
    ...
    *)
        ...
        echo "  → Create/switch to feature/be/PLAN-001-user-auth branch"
        echo ""
        echo "  bash scripts/git-branch-helper.sh prepare fe-coding PLAN-001"
        echo "  → Create/switch to feature/fe/PLAN-001 branch"
        echo ""
        exit 1
        ;;
esac
